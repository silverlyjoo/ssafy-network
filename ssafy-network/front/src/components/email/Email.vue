  ...
  <v-container>
    <div>
      ...
      <v-data-table :headers="headers" :items="desserts" :expand="expand" item-key="_id">
        ...
        <template v-slot:expand="props">
          <v-card flat>
            <v-card-text>
              <v-layout>{{ props.item.content }}</v-layout>
              <v-btn @click="deleteMail(props.item._id)">메일 삭제</v-btn>
            </v-card-text>
          </v-card>
        </template>
      </v-data-table>
    ...
    <v-dialog v-model="dialog" persistent max-width="600px">
      <template v-slot:activator="{ on }">
        <v-btn color="primary" dark v-on="on">메일 쓰기</v-btn>
      </template>
      <v-card>
        <v-card-title>
            ...
            <v-layout wrap>
              <v-flex xs12 sm6 md4>
                ...
                <v-select
                  ...
                  style="max-width:20vh;"
                ></v-select>
                <v-btn @click="addreceive">수신인 등록</v-btn>
                <v-btn @click="removereceive">수신인 삭제</v-btn>
              </v-flex>
              <v-flex xs12>
        ...
        <v-card-actions>
          ...
          <v-btn color="blue darken-1" flat @click="send">발송</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-container>
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "시간", value: "createdAt", sortable: false }
      ],
      desserts: []
    };
  },
